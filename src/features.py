...
def build_features(df_clean: pd.DataFrame) -> Tuple[pd.DataFrame, pd.Series]:
    ...
    np.random.seed(42) # Ensure determinism for reproducibility
    is_winner_p1 = np.random.rand(len(df_features)) > 0.5
    
    # 2. Extract features without leakage
    # We create p1_* and p2_* features based on the random assignment
    
    # Ranks
    df_features['p1_rank'] = np.where(is_winner_p1, df_features['winner_rank'], df_features['loser_rank'])
    df_features['p2_rank'] = np.where(is_winner_p1, df_features['loser_rank'], df_features['winner_rank'])
    ...
    if 'winner_age' in df_features.columns and 'loser_age' in df_features.columns:
        p1_age = np.where(is_winner_p1, df_features['winner_age'], df_features['loser_age'])
        p2_age = np.where(is_winner_p1, df_features['loser_age'], df_features['winner_age'])
        df_features['age_diff'] = p1_age - p2_age
        
    # Height (demographics)
    if 'winner_ht' in df_features.columns and 'loser_ht' in df_features.columns:
        p1_ht = np.where(is_winner_p1, df_features['winner_ht'], df_features['loser_ht'])
        p2_ht = np.where(is_winner_p1, df_features['loser_ht'], df_features['winner_ht'])
    ...
    if 'winner_hand' in df_features.columns and 'loser_hand' in df_features.columns:
        df_features['p1_hand'] = np.where(is_winner_p1, df_features['winner_hand'], df_features['loser_hand'])
        df_features['p2_hand'] = np.where(is_winner_p1, df_features['loser_hand'], df_features['winner_hand'])
    ...
    leakage_cols = [c for c in df_features.columns if c.startswith('winner_') or c.startswith('loser_') or c.startswith('w_') or c.startswith('l_')]
    ...
    df_features = df_features.drop(columns=[c for c in id_cols if c in df_features.columns])
